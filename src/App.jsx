...
function useJuhyuCalc() {
  ...
  useEffect(() => {
    try { localStorage.setItem(STORAGE_KEY, JSON.stringify({ wage, hours, fullAttendance })); } catch (e) { /* ignore */ }
  }, [wage, hours, fullAttendance]);
...
function Calculator() {
  const {
    wage, setWage, hours, setDayHour, fullAttendance, setFullAttendance,
    weeklyHours, eligibleByHours, eligible, juhyuPay, basePay, totalPay,
  } = useJuhyuCalc();
...
function Articles() {
  ...
  const filtered = query
    ? ARTICLES.filter((a) => a.t.toLowerCase().includes(query) || a.p.some((p) => p.toLowerCase().includes(query)))
    : ARTICLES;
  ...
  return (
    <div>
      ...
      <input
        ...
        style={{
          width: '100%', background: 'transparent', borderRadius: 0, padding: '9px 2px', fontSize: 13,
          color: C.ink, border: 'none', borderBottom: `1.5px solid ${C.lineStrong}`, boxSizing: 'border-box', marginBottom: 16,
        }}
      />
      {filtered.map((a, i) => (
        <details key={i} style={{ borderBottom: `1px dashed ${C.line}`, padding: '2px 0' }}>
          <summary style={{ padding: '11px 2px', fontSize: 13, fontWeight: 700, color: C.ink, cursor: 'pointer', display: 'flex', gap: 8 }}>
            <span style={{ color: C.inkFaint, fontFamily: "'Nanum Gothic Coding', monospace", fontSize: 11 }}>
              {String(i + 1).padStart(2, '0')}
            </span>
            {a.t}
          </summary>
          <div style={{ paddingBottom: 14, paddingLeft: 26 }}>
            {a.p.map((para, j) => (
              <p key={j} style={{ fontSize: 12.5, lineHeight: 1.85, color: C.inkSoft, margin: j === 0 ? '2px 0 10px' : '0 0 10px' }}>{para}</p>
            ))}
          </div>
        </details>
      ))}
        ...
        <div style={{ boxShadow: '0 18px 40px rgba(22,24,27,0.16)' }}>
          ...
          <div style={{ background: C.paper, padding: '22px 26px 8px' }}>
            ...
            <div style={{ display: 'flex', marginBottom: 20, position: 'relative' }}>
              {[{ v: 'calc', t: '계산기' }, { v: 'guide', t: '공부방' }].map((o, idx) => {
                const on = tab === o.v;
                return (
